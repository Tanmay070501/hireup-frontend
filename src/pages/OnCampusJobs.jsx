...
function OnCampusJobs() {
    ...
    const fetchJobList = useCallback(async () => {
        setLoading(() => true);
        try {
            const res = await axiosInstance.get("/student/oncampus");
    ...
    useEffect(() => {
        if(user?.banByPCell) return;
        fetchJobList();
    }, [fetchJobList, user]);
    if(user?.banByPCell)return  <Container maxWidth={"lg"}>
    <Typography
        variant="h4"
        style={{ margin: "20px auto", textAlign: "center" }}
    >
        You are banned by your Placement Cell.
    </Typography>
</Container>
